...
import com.itsamirrezah.goscore.data.db.GoscoreDb
import com.itsamirrezah.goscore.data.models.Team
import com.itsamirrezah.goscore.data.services.FootbalDataApiImp
import com.itsamirrezah.goscore.ui.items.*
import com.itsamirrezah.goscore.ui.model.*
import com.itsamirrezah.goscore.util.EndlessScrollListener
...
class MainActivity : AppCompatActivity() {
    ...
    private suspend fun catchIOException(block: suspend () -> Unit) {
        try {
            ...
            Log.d(com.itsamirrezah.goscore.util.TAG, "catchIOException: ${e.message}")
        }
    }
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    FootbalDataApiImp.getApi().getTeamsByCompetition(id.toInt())
                        .also {
                            ...
                            GoscoreDb.getInstance(this@MainActivity).teamsDao()
    ...
    private val onTeamInfo: OnTeamInfo = object : OnTeamInfo {
        ...
        override fun getTeamsFlag(homeTeamId: Int, awayTeamId: Int, onResult: OnResult) {
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                val homeTeam = async {
                    GoscoreDb.getInstance(this@MainActivity).teamsDao().getTeamById(homeTeamId)
                }
                val awayTeam = async {
                    GoscoreDb.getInstance(this@MainActivity).teamsDao().getTeamById(awayTeamId)
                }
